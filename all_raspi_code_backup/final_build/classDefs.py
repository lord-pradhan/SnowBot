...
class Controller:
    ...
    def __init__(self, snowbot, path, mark):
        ...
        #control parameters
        self.L = 0.5  #bounding box length
        self.k_p = 1.0  #gain
        # self.dt_ctrl = 0.05
        self.v_target = 1.5
...
##        self.in_flag = 0
    ...
    def control_update(self, snowbot, path, drivearduino, mark, mode = 0):
        ...
        if mode ==0: # normal operation
            ...
            self.x_track = Path(snowbot, mark).x_d[wrappath(gamma_vtp, path)]
            self.y_track = Path(snowbot, mark).y_d[wrappath(gamma_vtp, path)]
        ...
        # for dump manouver
            ...
            if d_min > L_dump:
                ...
                self.x_track = x_d_ref[gamma_min]
                self.y_track = y_d_ref[gamma_min]
        ...
        # go home manouver
            ...
            if (d_home > 0.1):
                ...
                self.x_track = snowbot.home_x
                self.y_track = snowbot.home_y
...
class SensorFusion:
    ...
    R_ekf = np.array([[1e-3, 0, 0], [0, 1e-3, 0], [0, 0, 1e-3]])
    # J_w = np.eye(3)
    x0_ekf = 0.3
...
class Path:
    ...
    def __init__(self, snowbot, mark=1):
        ...
        self.a_el1 = self.a_boundary - mark*0.8#snowbot.c;  # major axis
        self.b_el1 = self.b_boundary - mark*0.8#snowbot.c;  # minor axis
